...
- **Fabricated evidence.** Invented testimonials with stock-photo-adjacent names, fictional client logos, axisless charts, and implausibly tidy metrics ("10x faster · 99.9% uptime · 4.9★") are now actively recognized and destroy trust. Every person, quote, number, and logo must come from the client or be visibly labeled demo/placeholder where it appears, then flagged in the delivery report. Never use a green live dot, “today” label, animated ticker, or real-time language on invented data. Never invent a named human.
...
- **The component-catalog page silhouette.** Using strong components is not slop; dropping their demo page composition into every project is. Keep the component's valuable interaction, motion, state model, or distinctive treatment, but place it inside the Reference Blend Contract's silhouette, focal hierarchy, media strategy, density, and scroll story. Adapt demo spacing, geometry, and surrounding layout when they conflict with that contract. A component should make an authored composition richer, not decide that every site needs the same split hero, card wall, floating dashboard, or pill-and-glow ensemble.
- **Micro-detail standing in for a dominant reference trait.** Matching a reference's nav height, radius, or accent while discarding the full-bleed image, major animation, typography hierarchy, or density the user actually praised is reference-washing. The Liked Trait Ledger is the test: dominant traits must survive at dominant scale.
- **Component collage without art direction.** Individually polished components can still create a generic page when each brings a different demo anatomy or motion idea. The Reference Blend Contract sets one hierarchy and motion character; selected components are adapted into it instead of arranged as a catalog sampler.
...
3. **Macro-composition scan.** Look at the page zoomed out beside the approved references. Check silhouette, first focal point, media dominance/crop, type scale, density shifts, and scroll rhythm. A match in nav/color/radius does not pass when the dominant reference traits are missing. Then scan for generic formulas such as centered hero + 3 cards + testimonial + CTA band or split hero + floating demo + stats strip.
...
6. **Evidence check.** Every testimonial, logo, statistic, and chart: client-provided, or visibly labeled demo/placeholder on the rendered page and flagged in the report. Zero invented named humans; zero fake live indicators or real-time language.
...
9. **Liked-trait dominance check.** Audit every Liked Trait Ledger item. Confirm dominant likes remain dominant in the render rather than being replaced by easier micro-details.
10. **Component-composition check.** Confirm premium components are used where applicable, but share one reference-led hierarchy instead of reproducing a catalog demo page or a collage of unrelated component aesthetics.
11. **Signature element check.** The brief names one signature element; confirm it exists in the build and actually reads as distinctive.
12. **Execution-bug sweep (on rendered pages, not code).** Walk the execution-bug list: no content that stays invisible if a reveal misfires, no clipped or edge-jammed text, comparison columns aligned on a shared grid, optical centering verified, no color seams or unreadable text, no blur/shadow/glow applied where it has nothing to act on.
13. **Report.** The self-review report includes a "slop audit" line: what was caught, what was fixed, anything deliberately kept and why.
