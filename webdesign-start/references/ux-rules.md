...
- **Every designed motion surface needs best-fit catalog lineage.** Follow `component-sourcing.md`: compare the strongest viable candidate from every catalog without source priority, then use the winner directly or preserve its meaningful motion/interaction craft in a faithful adaptation. Apply `threeui.md` runtime gates to ThreeUI finalists. Library provenance does not excuse failed accessibility, reduced motion, or renderer lifecycle.
