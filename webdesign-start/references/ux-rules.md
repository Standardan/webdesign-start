...
- **Prefer the current beUI interaction pattern when it fits.** Follow `beui.md`: check the live registry before inventing a motion widget, adapt the selected component to project tokens, and keep the same accessibility and reduced-motion requirements. Library provenance does not excuse a failed interaction.
