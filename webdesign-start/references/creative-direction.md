...
11. **Mobile recomposition.** What changes and what stays prominent on a phone ("the speaker stays at 50% of the screen in every chapter; turns become four stills").
...
**A speaker launch (product register, product film):**

> Design "Halden One — Quiet, Until It Isn't", an Apple-style launch film for a fictional compact wireless speaker, in a Swiss-precise product language. Ground: studio white #fbfbfd with a soft grey floor #eceef1; palette: graphite ink #111316, anodised sage #8fa89a (the speaker's own finish), cool grey #6b7178 for secondary text, and signal orange #ff6a2b used only for the Buy button and focus. Hero: the speaker, studio-lit from the upper left with a rim and a floor reflection, slowly turning as a 120-frame image sequence; "Quiet, until it isn't." in a tight neo-grotesk at 12vw sits behind it, and the speaker's shadow crosses the letters. Sections: the design as a pinned turn that ends side-on to show it's 38 mm thin; the sound as an explode that separates grille, driver and passive radiator; battery as one huge number, "30 h", that counts once with its test footnote; finishes as a live swap across three colours with swatch radios; specs as a clean compare table against last year's model. Key components: a GSAP-pinned scene for each chapter; a Magic UI number ticker restyled in tabular Inter Display; a shadcn sheet for the buy drawer. Interaction: "Turn it" (drag to rotate the speaker in the hero). Delight: the grille lights a single orange ring when you hover Buy. Typography: Inter Display 600 at 12vw with −0.045em tracking for headlines; Inter 400 at 21px for supporting lines; numbers in tabular figures. Mobile: the speaker holds 50% of each chapter, turns become four stills, and the buy bar sticks to the bottom. Calm, exact and confident; no scenery, no time of day, one claim per chapter.

**An invoicing app dashboard (interface register):**

> Design "Ledgerline — Know Where Every Invoice Stands", the customer dashboard of a fictional invoicing app, in a calm editorial-data language. Ground: paper white #fafaf9 (dark theme: ink #111214), surfaces #ffffff, hairlines #e7e5e4; palette: ink #1c1917, muted #78716c, accent iris #5b5bd6 for primary actions and selection, plus paid green #1f9d55, overdue red #d64545 and pending amber #c98a13, all checked for contrast on both themes. Hero: the one number, "£18,420 outstanding", at 48px with its change ("£2,100 less than last week, 3 invoices paid") beside a slim 12-week bar chart. Sections: a working list of invoices with status chips, right-aligned tabular amounts and sortable columns; an "overdue" lane that surfaces the three invoices to chase today; a detail drawer that slides in from the right with the timeline of each invoice. Key components: a shadcn table and sheet restyled to the tokens; a command palette (⌘K) for "new invoice", "find client" and "mark paid". Interaction: "Mark paid" (the row settles into the paid group with an undo toast). Delight: the outstanding number rolls down smoothly when an invoice is paid. Typography: Inter at 13–15px for UI, 48px for the hero number, tabular numerals throughout. Mobile: a bottom tab bar, the hero number first, and invoices as cards with client, amount and status. Calm and precise; demo data labelled; nothing bounces.
