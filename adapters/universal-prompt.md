...
1. Harvest what I've already told you; don't re-ask it. On every invocation, attempt the self-update check in `SKILL.md` and the live beUI refresh in `webdesign-start/references/beui.md`.
...
4. Turn every approved reference trait into a Reference Translation Matrix. Check every meaningful feature against the current beUI registry and add a Component Opportunity Map with live candidates, recommendation, target, adaptation, and constraints. Synthesize both into `DESIGN-BRIEF.md` using `webdesign-start/references/brief-template.md`. Show me a short digest and STOP for my approval.
5. Only after I approve: automatically use the strongest current beUI matches wherever they fit, and build a reference-driven style sample containing the highest-impact selection or native-stack adaptation. Do not show me a component picker first. Pause for my critique or confirmation, then build the full site against the brief, following `webdesign-start/references/build-standards.md`, `references/beui.md`, and `references/ux-rules.md`.
